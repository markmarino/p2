...
{
   $num_words = $_GET['number_of_words']; #set value from GET
...
{
   $case_mode = $_GET['case_select']; #set value from GET
}
if(isset($_GET['separator_select']) && !empty($_GET['separator_select']))
{
    if ($_GET['separator_select'] == "hyphen") {
        $separator_char = "-";
    } else if ($_GET['separator_select'] == "space") {
        $separator_char = " ";
    } else if ($_GET['separator_select'] == "No separator") {
        $separator_char = "";
    }
}
...
for($i = 0; $i<$num_words; $i++){
    ...
    # append each word to the $generated_password string, with a separator character between
    ...
    $generated_password .= $current_word . $separator_char;
}
#trim last extraneous separator character from the end of the $generated_password string
$generated_password = rtrim($generated_password,$separator_char);
...
if (isset($_GET['add_number'])) {
        ...
        $generated_password .= $separator_char . $random_number;
}
if (isset($_GET['add_symbol'])) {
        $random_symbol = random_char($symbols);
        $generated_password .= $separator_char . $random_symbol;
}
